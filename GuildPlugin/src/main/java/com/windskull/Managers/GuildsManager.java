...
public class GuildsManager
{
	...
	private Map<Guild, GuildManager> allGuildsManagers = new HashMap<>();

	private Map<UUID, GuildPlayer> allGuildPlayers = new HashMap<>();
	public Map<Player, PlayerInvitations> playersInvitations = new HashMap<>();
	...
	{
		return new ArrayList<Guild>(this.allGuilds);
	}
	...
	public GuildManager getGuildManager(Guild g)
	{
		...
		{
			System.out.println("New guild manager for: " + g);
	...
	{
		if (playersInvitations.containsKey(p))
		{
			playersInvitations.get(p).addInvitation(g);
		} else
		{

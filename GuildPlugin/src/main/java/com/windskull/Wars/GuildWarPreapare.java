...
public class GuildWarPreapare implements Listener
{
	...
	private List<Inventory_GuildMenu_WarPanel> watchers = new ArrayList<>();
	 ...
	 * {
	 * Optional<GuildPlayer> player = queuePlayers.stream().filter(gp ->
	 * gp.getPlayeruuid().equals(e.getPlayer().getUniqueId())).findFirst();
	 * if(player.isPresent())
	 * {
	 * player.get().init();
	 * }
	...
	public void onPlayerLeave(PlayerQuitEvent e)
	{
		...
		if (player.isPresent())
		{
			switch (guildQueueStatus)
			{
				case INQUEUE:
					GuildsWarsQueue.getInstance().leaveQueue(this);
					queuePlayers.forEach(pl ->
					{
						if (pl.getPlayer() != null)
						{
							pl.getPlayer().sendMessage(GuildsManager._ItemsColorNamePrimal + "Opuszczono kolejke");
						}
					});
					break;
				default:

...
public class Inventory_GuildMenu_Buildings extends Inventory_GuildMenu
{
	...
	private void createInventory()
	{
		...
		//Entry<String, GuildBuilding>[] buildings = (Entry<String, GuildBuilding>[]) GuildBuildingsManager.getInstance().guildsBuildings.entrySet().toArray();
	...
	private ItemStack getGuilBuildingItem(GuildBuilding gb)
	{
		...
		ItemBuilder ib = new ItemBuilder(ItemsCreator.getItemStack(gb.materialIcon , GuildsManager._ItemsColorNamePrimal +  gb.buildingName +": "+GuildsManager._ItemsColorNameSecond + blvl));
		if(blvl < gb.maxBuildingLevel)
			gb.itemsforBuilding.get(blvl).forEach(gi -> ib.lore(GuildsManager._ItemsColorNamePrimal +gi.getName() + ": "+GuildsManager._ItemsColorNameSecond + gi.getAmount()));
		else
	...
	private void upgradeBuilding(int slot,GuildBuilding gb)
	{
		...
		if(tmpGuildBuilding.isPresent())
		{
			...
			if(tmpb.buildingLevel < gb.maxBuildingLevel)
			{
				...
				if(clipboard == null) player.sendMessage(GuildsManager._GlobalPrefix + "Nie udalo sie wkleic budynku");
				else
					try 
					{

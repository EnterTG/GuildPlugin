...
public class Inventory_GuildMenu_NewPlayer extends Inventory_GuildMenu
{
	...
	private void openNameInput()
	{
		new AnvilGUI.Builder().onClose(player ->
		{
			if (this.name == null)
			{
				player.sendMessage(GuildsManager._GlobalPrefix + "Tworzenie gildi przerwane");
			}
		}).onComplete((player, text) ->
		{
			if (text.length() > 3 && text.length() <= 20)
			{
				if (!GuildsManager.getGuildManager().isGuildExist(text))
				{
					this.name = text;
					this.openTagInput();
					return AnvilGUI.Response.text("Zakceptowano");
				} else
				{
					return AnvilGUI.Response.text("Nazwa zajeta");
				}
			} else
			{
				return AnvilGUI.Response.text("Nazwa musi zawierac od 4 do 20 znakow.");
			}
		}).text("Podaj nazwe gildi").title("Podaj nazwe gildi").plugin(GuildPluginMain.main).open(this.player);
	}
	...
	private void openTagInput()
	{
		...
		}).onComplete((player, text) ->
		{
			...
			{
				if (!GuildsManager.getGuildManager().isTAGExist(text))
				{
					this.tag = text;
				...
				{
					return AnvilGUI.Response.text("Tag zajety");
				}
			} else
			{
				return AnvilGUI.Response.text("Tag musi zawierac od 2 do 4 znakow.");
			}
		}).text("Podaj TAG").title("Podaj TAG").plugin(GuildPluginMain.main).open(this.player);
	}
	...
	private void createInventory_Invitation()
	{
		...
		if (pi != null)
		{
			...
			IntStream.range(0, newList.size())
				.forEach(i -> this.setItem(playerContainer[i], ItemsCreator.getItemStack(Material.NETHER_STAR, GuildsManager._ItemsColorNamePrimal + "LPM zeby dolaczyc do: " +GuildsManager._ItemsColorNameSecond + newList.get(i).getName() ),
					e -> setGuild(newList.get(i))));
			buttons_NextPrevPage(newList.size(),() -> createInventory_Invitation());
